# Rushil-Portfolio

## About Me

I’m a Computer Science undergraduate passionate about **Artificial Intelligence, Robotics, IoT, and Automation**.

I enjoy building technology-driven solutions that solve real-world problems and contribute to smarter, more efficient systems. My interests lie in combining **AI, embedded systems, and intelligent automation** to create impactful applications in domains such as agriculture and robotics.

I continuously explore emerging technologies and work on expanding both my technical knowledge and practical development skills.

---

## Education

### Manipal University Jaipur *(Aug 2024 – Present)*

**Bachelor of Technology in Computer Science**  
CGPA: **8.6/10**

Relevant Coursework:

* Data Structures & Algorithms
* Operating Systems
* Machine Learning
* Internet of Things (IoT)
* Embedded Systems

---

### SNBP International School *(Jun 2022 – May 2024)*

Class XII — **76%**

---

### The Bishop's Co-Ed School, Kalyaninagar *(Jun 2009 – May 2022)*

Class X — **95%**

---

## Skills

* **Programming Languages:** Python, C/C++, Java
* **AI/ML & Libraries:** OpenCV, NumPy
* **Backend, APIs & Databases:** Flask, REST APIs, Firebase, SQLite
* **Hardware/IoT:** Arduino, Raspberry Pi, IR Sensors

---

## Projects

### AgriDetect — Smart Crop Monitoring System

* Developing an intelligent crop monitoring system focused on **Wheat, Rice, and Cotton** crops.
* Utilizing **image processing and machine learning** techniques for crop disease detection.
* Planning integration with **IoT sensors** for real-time environmental monitoring.
* Designing scalable architecture to support future agricultural deployment and additional crops.
* Focused on improving farmer decision-making and reducing crop loss.

**Tech Stack:** Python, OpenCV, NumPy, Flask, Firebase, SQLite, REST APIs

---

### Line Following Robot (LFR)

* Designed and developed an autonomous robot capable of following predefined paths using IR sensors.
* Implemented real-time feedback control using Arduino for motor control and navigation.
* Optimized robot movement across curves and intersections with improved path accuracy.
* Participated in a robotics competition and gained practical experience in embedded systems.

**Tech Stack:** Arduino, Embedded C, Arduino IDE

---

## Achievements

* Ranked 14th on the Kagglethon AI/ML competition leaderboard.
* Built and competed with a Line Following Robot in a robotics competition.
* Maintaining a strong academic record with a CGPA of 8.6/10.
* Actively exploring AI, robotics, and drone technologies through self-driven learning and projects.

---

## Interests

* Artificial Intelligence
* Computer Vision
* Automation
* Robotics
* IoT

---

## Connect with Me

* 📧 Email: [rushilgupta706@gmail.com](mailto:rushilgupta706@gmail.com)
* 🌐 GitHub: [shyguy-0706](https://github.com/shyguy-0706)
* 💼 LinkedIn: [Rushil Gupta](https://www.linkedin.com/in/rushil-gupta-muj-aiml/)

